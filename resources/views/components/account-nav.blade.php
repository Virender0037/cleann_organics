@props(['active' => null])
{{--
    Shared customer-account sidebar (Phase J). Replaces the ~160 lines of
    identical markup previously copy-pasted across user-dashboard,
    order-history, account-setting and orders/show. `active` is one of:
    dashboard | orders | settings | addresses | wishlist.
--}}
<nav class="dashboard__nav">
    <h5 class="dashboard__nav-title font-body--xxl-500">navigation</h5>
    <ul class="dashboard__nav-item">
        <li class="dashboard__nav-item-link {{ $active === 'dashboard' ? 'active' : '' }}">
        ...
        <li class="dashboard__nav-item-link {{ $active === 'orders' ? 'active' : '' }}">
        ...
        <li class="dashboard__nav-item-link {{ $active === 'settings' ? 'active' : '' }}">
        ...
        <li class="dashboard__nav-item-link {{ $active === 'addresses' ? 'active' : '' }}">
            <a href="{{ route('account-setting') }}#addresses" class="font-body--lg-400">
                <span class="icon">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M12 21C12 21 4 15 4 9.5C4 7.5 5 5.5 7 4.5C8.5 3.75 10.5 3.75 12 4.5C13.5 3.75 15.5 3.75 17 4.5C19 5.5 20 7.5 20 9.5C20 15 12 21 12 21Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                        <circle cx="12" cy="9.5" r="2.5" stroke="currentColor" stroke-width="1.5" />
                    </svg>
                </span>
                <span class="name"> My Addresses </span>
            </a>
        </li>
        <li class="dashboard__nav-item-link {{ $active === 'wishlist' ? 'active' : '' }}">
        ...
        <li class="dashboard__nav-item-link">
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="font-body--lg-400" style="background:none;border:0;padding:0;width:100%;display:flex;align-items:center;cursor:pointer;color:inherit;font:inherit;">
                    <span class="icon">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    {{-- Tablet/mobile toggle (≤991px the sidebar is fixed off-screen and
         only .dashboard__nav.active reveals it) — wired in main.js. --}}
    <button class="filter-icon" type="button" aria-label="Toggle account navigation">
