...
- **Production DB is MariaDB with `ONLY_FULL_GROUP_BY`; never `GROUP BY products.id` over `products.*`** (error 1055 — MariaDB has no functional-dependence detection, unlike the SQLite test DB and unlike MySQL 5.7+). The shop's price/rating filters are plain `WHERE` conditions on correlated subqueries (`ProductCatalogService::EFFECTIVE_PRICE_SUBQUERY` / `APPROVED_RATING_SUBQUERY`), never `GROUP BY`/`HAVING`. `ShopTest` asserts the listing SQL contains neither (SQLite can't catch this by results). Local dev DB is also MariaDB with that mode, so `/shop?max_price=50` reproduces it locally.
- **Live `public/` is a separate copy: `~/domains/cleannorganics.com/public_html` ≠ repo `~/domains/cleannorganics.com/cleann_organics/public`.** `git pull` updates Blade/PHP but NOT the served CSS/JS. After any change under `public/css` or `public/js`, copy those files to `public_html` (never assume). New HTML + old CSS once blew the site out (a 1600px logo and a 1080px product image rendered at natural size). Defences now in code: new-component images carry inline `max-width`/`max-height` guards, and the storefront `style.css`, `home1.js`, `cart.js` load through `admin_asset()` (filemtime `?v=`). `public/js/wishlist.js` was never synced originally (404 on live) — verify with a scan of asset URLs when in doubt.
- **Product-card image-link CSS must stay child-scoped** (`.cards-md__img-wrapper > a`, and the `--two/--three/--four` variants). The descendant form `… a` also matched the quick-view eye link (`<a class="action-btn">`) and stretched it into a 40×300 green pill. `StorefrontLayoutTest` guards this.
- **Header/footer logos are elastic** (`height:auto; max-height; max-width:min(…,100%)`, `min-width:0` up the flex chain) so a wide uploaded logo can't push the cart off a 320px screen. The uploaded logo canvas often has large transparent margins (live one: artwork = 25% of image height) — crop the file rather than raising CSS sizes.
- **Hero frame follows the uploaded art, never a fixed banner ratio.** `.home-hero__media` uses `aspect-ratio: var(--hero-ratio)` (desktop) / `var(--hero-ratio-m)` (≤767px), computed from the FIRST slide's real image via `HomeBanner::imageRatio()` (cached per file version, clamped 1.4–3.2 / 0.75–1.8, fallback 8:3). A fixed 8:3 frame used to crop 16:9 uploads by a third and cut embedded text. Per-slide `text_position` (`left|right`, migration `2026_09_21_100000`) keeps the text card off the subject; a slide with no title/subtitle renders only the CTA button (`home-hero__text--cta-only`, no white card). Never scale/transform hero text.
- **Template leftovers are regressions, not content**: the About "team" (Cody Fisher…) is now `TeamMember` rows (section hidden when none), the About feature cards are the real Benefits Strip, the fake brand-logo strip (home + About), the blog "Our Gallery" tiles, the hardcoded homepage news fallback ("Curabitur…", `single-blog.html`) and `*.html` breadcrumb links are all gone. Homepage "Latest News" = external feed → else latest published `Blog` posts → else hidden. `StorefrontContentTest` guards all of this. Blog slugs on production are raw titles with spaces/quotes (data issue, still routable) — slugify new posts.
- **Product cards**: image frame is a square (`aspect-ratio: 1/1`, not a fixed pixel height — that made the ratio vary 0.86–1.35 across widths); card actions (wishlist/eye) are always visible on touch/≤991px (`@media (hover: none), (max-width: 991px)`); names reserve two lines (`min-height: 2lh`) so card heights stay equal.
- **Admin order print**: `GET /admin/sales/orders/{order}/print` (standalone view `admin/sales/orders/print.blade.php`, all CSS inline → no public asset to sync; `?auto=1` opens the print dialog). Reads stored order/payment/address-snapshot data only; older orders whose GST was added on top are labelled correctly.
- Responsiveness audit method that works: headless Chrome over CDP (`--remote-debugging-port`), 8 widths (1440–320) × guest+logged-in pages, measuring `documentElement.scrollWidth` vs viewport plus offender elements. Off-screen Swiper slides and off-canvas drawers are false positives; `scrollWidth == viewport` is the real test.
...
## Marketplace price comparison (added 2026-09-22)

Optional "Compare prices" beside our own price. Prices are **typed in by an admin** — nothing is scraped or fetched, and the UI never claims they are live.

- **Data**: `product_marketplace_prices` (product, optional `product_variant_id`, `marketplace` key, `selling_price`, `mrp`, `product_url`, `affiliate_url`, `is_active`, `sort_order`, `last_checked_at`) + `marketplace_clicks` (product, price, marketplace, source, created_at — **no IP/UA/user**). Uniqueness is (`product_id`, `variant_scope`, `marketplace`) where `variant_scope` is a NOT NULL mirror of the nullable variant id (0 = product-level), set in the model's `saving` hook — a unique index over a nullable column allows duplicates on both MariaDB and SQLite.
- **One source of truth**: `config/marketplaces.php` (`list`: label/enabled/domains, plus `click_sources`, `stale_after_days`) read via `App\Support\Marketplaces`. Never branch on "amazon"/"flipkart" in views. Adding Blinkit etc. = a config entry, no migration. Logos: drop `public/images/marketplaces/{key}.svg|png|webp|jpg` and `<x-frontend.marketplace-mark>` uses it; otherwise a fixed-height text wordmark (never hotlinked).
- **Variant rule** (`App\Services\Storefront\MarketplaceComparison::offersFor`): per marketplace, a listing for the exact variant wins, else the product-level listing. Only *visible* rows count (`Product::visibleMarketplacePrices` = active + priced; a disabled variant-specific row simply stops existing, so the product-level one then applies to that variant). Own price = `ProductVariant::headlinePrice()` (no pricing logic duplicated). **"Best Price" only when our price is strictly lower than every visible offer**; otherwise just "Official Store".
- **Storefront**: PDP block (`components/frontend/compare-offers`, mode `pdp`) is server-rendered for the default variant and every variant ships as `<template data-compare-variant>`; `public/js/compare-prices.js` swaps it on variant change (`ProductController::show` builds `comparePayloads`; `selectVariant()` calls `CompareOffers.showVariant`). Product cards (`<x-frontend.product-card source="…">`) get a "Compare prices" button + `<template>` only when the list eager-loaded `visibleMarketplacePrices` (**every product list must `with('visibleMarketplacePrices')`** — the card never lazy-loads); ONE shared native `<dialog>` is created by JS on first use (bottom sheet ≤575px). No offers ⇒ no section, button or dialog. No structured data / SEO change.
- **Outbound links**: only `GET /out/marketplace/{id}?src=…` (`MarketplaceRedirectController`): requires an active, priced, enabled-marketplace record on a public product, redirects to its stored `affiliate_url ?? product_url` (http/https only), logs a click (src whitelisted, else `product_card`), never reads a URL from the request, `noindex` + `no-store`, throttled. Links carry `rel="noopener noreferrer nofollow sponsored"`; raw destinations are never printed in page HTML.
